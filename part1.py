...
class rnn(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        """
        # each sequence, with each batch input
        for each_input in input:
            hidden = self.rnnCell(each_input, hidden)
...
def lstm(input, hiddenSize):
    ...
    """
    batchSize, seqLength, inputDim = input.shape
    lstm = torch.nn.LSTM(inputDim, hidden_size=hiddenSize, batch_first=True)
    return lstm(input)
