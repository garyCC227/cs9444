...
class NetworkCnn(tnn.Module):
    ...
    def forward(self, input, length):
        ...
        x = input.permute(0, 2, 1) #[bath, inputdim, seq Length]
...
def measures(outputs, labels):
    ...
    tp = torch.sum(torch.eq(outputs * labels, 1)).item()
    fn = torch.sum(torch.eq(outputs - labels, -1)).item()
    fp = torch.sum(torch.eq(outputs - labels, 1)).item()
    tn = torch.sum(torch.eq(outputs + labels, 0)).item()
    return tp, tn, fp, fn
